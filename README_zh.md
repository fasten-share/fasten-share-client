...
客户端底层最多可隔离保存并运行 5 个已登录账号，各账号的后端、上游 API Key、自动共享设置和生产连接互不影响。客户端界面不提供添加账号或账号切换功能；账号变化只通过正常登录流程发生。退出账号会删除登录凭据并停止该账号连接，但保留配置供同一账号重新登录后恢复。V2 多账号存储不会读取或迁移旧版单账号配置。
...
5. 选择协议和版本前缀。
6. 如果当前协议支持转换，可选择目标协议或“不转换”，再按最终目标协议选择消费者工具。
7. 填写用于搜索发现的模型名称。
8. 设置并发数和积分倍率。
9. 保存并开始共享。
